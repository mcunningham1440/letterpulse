...
def _run_learning_task_impl(task, kind):
    ...
    if kind == 'initial':
        ...
        # so we don't append to a half-finished dataset. Also resets
        # UserPublication.initial_fetch_done_at back to null.
    ...
    if not post_ids_to_process:
        # Empty run — for kind='initial' we don't stamp
        # UserPublication.initial_fetch_done_at so the user can retry via the
        # Learning coach on their next visit.
        task.mark_complete()
        return
    ...
    # Only stamp UserPublication.initial_fetch_done_at after a clean,
    # non-empty run so a zero-result initial scan doesn't strand the user on
    # the "No Data" fallback. wipe_user_publication_data at the start of the
    # run already cleared it, so this re-stamps it on successful completion.
