...
When `ENVIRONMENT == 'local'`, a floating panel shows per-call details. `llm_tracker.py` is a contextvar-based accumulator; sync workflows return data in JSON, background workflows store it in `dev_panel_data` on Pending* models. `static/analytics/js/dev-panel.js` renders, loaded via `{% if is_local %}`. Each call shows a provider chip (`openai` / `anthropic`); fallback rows display "↩ fell back from <model>".

### Multi-provider `llm_call`
`utils.llm.llm_call` dispatches to OpenAI or Anthropic by model-name prefix (`gpt-*` → openai, `claude-*` → anthropic — see `utils.llm_providers.base.provider_name_for`). Adapters live in `utils/llm_providers/{openai_provider, anthropic_provider}.py` and translate to a `NormalizedResponse` (`output_parsed`, `output_text`, `output_items`, `tool_calls`, `usage`) so call sites are provider-agnostic. On a retryable failure (rate limit, timeout, 5xx, overloaded), the orchestrator transparently retries on the model in `settings.LLM_FALLBACK_MAP` (tier-mapped: gpt-5.4↔claude-sonnet-4-6, gpt-5.4-mini↔claude-haiku-4-5). Each provider's SDK runs with `max_retries=0` — the orchestrator owns the retry budget. Per-provider quirks: Claude is stateless (`store`/`previous_response_id`/`prompt_cache_key` are silently ignored); `prompt_cache_retention="24h"` maps to Claude's `cache_control ttl="1h"` (silent downgrade); forced `tool_choice` disables thinking on the Claude path (Claude rejects the combination). Canonical message format is OpenAI Responses items; the Anthropic adapter translates in both directions so mid-conversation provider switches preserve text + tool calls (reasoning/thinking blocks are dropped at the boundary). Failures and successes both write `LLMCall` rows; the fallback row carries `additional_info['fell_back_from']`.
...
`test_beehiiv_api.py` and `test_openai_provider.py` use **hand-written inline mocks** (Beehiiv via `aioresponses`; OpenAI patches `analytics.utils.llm_providers.openai_provider.AsyncOpenAI`). `test_anthropic_provider.py` patches `analytics.utils.llm_providers.anthropic_provider.AsyncAnthropic` for request-shape tests, and for response-translation tests **loads recorded API responses directly** via `Message.model_validate` / `ParsedMessage[Schema].model_validate` against fixture JSONs (one per code path: structured-output, free-form Sonnet, Haiku manual-budget thinking, two-turn tool-use). `test_llm_call.py` and `test_provider_fallback.py` stub providers at the `get_provider` boundary to exercise dispatch + fallback. OpenAI and Anthropic recordings (May 2026) live in `analytics/tests/fixtures/` (gitignored) — see `_record_phase2.py` and `_scrub.py`. To re-record: set `BEEHIIV_API_TOKEN`, `OPENAI_API_KEY`, `PERPLEXITY_API_KEY`, `ANTHROPIC_API_KEY` in `.env` (need pub with ≥11 posts), then `.venv/bin/python -m analytics.tests.fixtures._record_phase2 [--anthropic-only|--openai-only|--beehiiv-only] && .venv/bin/python -m analytics.tests.fixtures._scrub`. Recorder caches via per-file existence; `--force` to overwrite. **`anthropic.types.Usage.output_tokens_details` arrives via Pydantic `model_extra` as a plain dict in SDK 0.103** — `_extract_usage` handles both dict and object forms so a future SDK promotion doesn't break it.
